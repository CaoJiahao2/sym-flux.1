...
# Full-view MVS training stage: angle=[30,60]
# 默认配置：
...
#
# 可切换架构：
#   MV_ARCH=adapter     bash scripts/full_view/22_train_full_view_30_60.sh
#   MV_ARCH=full_hidden bash scripts/full_view/22_train_full_view_30_60.sh
#
...
export TRAIN_MANIFEST="${TRAIN_MANIFEST:-data/samples/stride_${FRAME_STRIDE:-8}_angle_30-60_v${NUM_VIEWS}_train_samples.jsonl}"
export INFER_MANIFEST="${INFER_MANIFEST:-data/samples/stride_${FRAME_STRIDE:-8}_angle_30-60_v${NUM_VIEWS}_val_samples.jsonl}"
...
RUN_NAME="${MV_ATTN_MODE}_angle_30-60_${MV_ARCH}_steps${MAX_STEPS}_views${NUM_VIEWS}_dim${MV_ADAPTER_DIM}"
export OUTPUT_DIR="${OUTPUT_DIR:-outputs/${RUN_NAME}}"
export INFER_OUT="${INFER_OUT:-${OUTPUT_DIR}/final_inference_angle_30-60.jpg}"

if [[ -z "${RESUME_MV_CKPT:-}" ]]; then
  PREV="outputs/full_view_angle_15-45_${MV_ARCH}/mv_adapter_last.pt"
  if [[ -f "${PREV}" ]]; then
    export RESUME_MV_CKPT="${PREV}"
    echo "[TRAIN] Auto resume from previous stage: ${RESUME_MV_CKPT}"
  fi
fi

echo "[TRAIN] angle=[30,60]"
