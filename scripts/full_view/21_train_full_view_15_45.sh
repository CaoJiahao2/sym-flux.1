...
# Full-view MVS training stage: angle=[15,45]
# 默认配置：
...
#
# 可切换架构：
#   MV_ARCH=adapter     bash scripts/full_view/21_train_full_view_15_45.sh
#   MV_ARCH=full_hidden bash scripts/full_view/21_train_full_view_15_45.sh
#
...
if [[ -z "${RESUME_MV_CKPT:-}" ]]; then
  PREV="outputs/full_view_angle_0-30_${MV_ARCH}/mv_adapter_last.pt"
  if [[ -f "${PREV}" ]]; then
